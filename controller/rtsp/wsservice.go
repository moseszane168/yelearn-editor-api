...
func (manager *clientManager) Start() {
	logrus.Info("Websocket manage start")
	for {
		select {
		case client := <-manager.register:
			logrus.Infof("Websocket client %s connect", client.ID)
			...
			logrus.Infof("Register client %s to %s group success", client.ID, client.Group)

		case client := <-manager.unRegister:
			logrus.Infof("Unregister websocket client %s", client.ID)
			if _, ok := manager.clientGroup[client.Group]; ok {
				if _, ok := manager.clientGroup[client.Group][client.ID]; ok {
					...
					logrus.Infof("Unregister websocket client %s from group %s success", client.ID, client.Group)

					if len(manager.clientGroup[client.Group]) == 0 {
						logrus.Infof("Clear no client group %s", client.Group)
...
func (manager *clientManager) RegisterClient(ctx *gin.Context) {
	...
	if err != nil {
		logrus.Errorf("websocket client connect %v error", ctx.Param("channel"))
		return
	}
